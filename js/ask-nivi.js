/* Ask NIVI — Farm 147 passport only, 10-minute key session */
(function () {
  ...
  function clearSession(silent) {
    ...
    keyInput.value = "";
    updateTimerUI();
    if (!silent) addMessage("nivi", "Session cleared. Paste a key and click Start 10 min to continue.");
  }

  function startSession(key) {
    const trimmed = (key || "").trim();
    if (!trimmed) {
      addMessage("nivi", "Paste a session key first, then click Start 10 min.");
      return;
    }
    try {
    ...
    addMessage(
      "nivi",
      "10-minute session started. Ask only about Farm 147 / this batch. Session auto-clears when time ends."
    );
  }
  ...
  function updateTimerUI() {
    ...
    tickTimer = setInterval(() => {
      if (!paint()) {
        clearInterval(tickTimer);
        tickTimer = null;
        addMessage("nivi", "10-minute session ended. Paste a key again to continue.");
      }
    }, 1000);
  }
  ...
  async function ask(question) {
    ...
    if (!sessionActive()) {
      addMessage("user", q);
      input.value = "";
      addMessage("nivi", "Start a 10-minute session: paste your key above and click Start 10 min.");
      return;
    }
